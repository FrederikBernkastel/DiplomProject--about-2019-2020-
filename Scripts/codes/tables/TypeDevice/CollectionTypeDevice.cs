...
{
    public partial class CollectionTypeDevice : BaseTable
    {
        public override string RUSNameTable { get { return "Типы устройств"; } }
        public override string ENGNameTable { get { return "type_device"; } }
    ...
    public partial class CollectionTypeDevice : BaseTable
    {
        ...
        {
            new ITypeDevice().ShowDialog();
        }
        public override void ShowFormUpdateInfo(IEnumerable<BaseData> indexes)
        {
            new UTypeDevice(indexes.Cast<TypeDevice>()).ShowDialog();
        }
                ...
                case "Устройства":
                    return SelectInfo().Cast<Device>().Where(u =>
                        ...
                        sdf[1] == "Кабинет" ?
                            ...
                            sdf[2] == "Не содержит" ? !u.Cabinet.ToString().Contains(sdf[3]) : true : true).Where(u => u.TypeDevice != null).Select(u => u.TypeDevice);
                default:
        ...
        {
            PrintInfo(view, RUSPrintField, list.Cast<TypeDevice>().Select(u => new object[]
            {
                u.ID,
                u.Name,
            }));
        ...
        public override IEnumerable<BaseData> SelectInfo()
        {
            foreach (DataRow s in NGDB.SelectInfo(ENGNameTable))
                yield return new TypeDevice().SetDataRow(s);
        }
    }
}
